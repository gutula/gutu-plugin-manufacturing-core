# Manufacturing Core Business Rules

## Invariants

- The plugin remains authoritative only for the data declared in `manufacturing-core` and its owned resource set.
...
- ERP parity references are tracked against: `Manufacturing`, `Subcontracting`.

## Lifecycle notes

- This plugin currently exports 7 action(s), 3 resource(s), 2 job definition(s), and 1 workflow definition(s).
- The domain catalog currently tracks 9 owned entity surface(s), 4 report surface(s), and 4 exception queue(s).
