export type ManufacturingCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildManufacturingCoreSqliteMigrationSql(options: ManufacturingCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "manufacturing_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getManufacturingCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getManufacturingCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildManufacturingCoreSqliteRollbackSql(options: ManufacturingCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "manufacturing_core_");
  return [
...
export function getManufacturingCoreSqliteLookupIndexName(tablePrefix = "manufacturing_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getManufacturingCoreSqliteStatusIndexName(tablePrefix = "manufacturing_core_"): string {
