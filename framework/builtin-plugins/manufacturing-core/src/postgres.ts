export type ManufacturingCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildManufacturingCoreMigrationSql(options: ManufacturingCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "manufacturing_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getManufacturingCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getManufacturingCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildManufacturingCoreRollbackSql(options: ManufacturingCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "manufacturing_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "manufacturing_core";
  return [
...
export function getManufacturingCoreLookupIndexName(): string {
  return "manufacturing_core_primary_lookup_idx";
}

export function getManufacturingCoreStatusIndexName(): string {
  return "manufacturing_core_exception_status_idx";
}
