...
describe("manufacturing-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "manufacturing-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "manufacturing-core:demo",
      title: "Manufacturing Core Demo",
      ...
      correlationId: "manufacturing-core:corr",
      processId: "manufacturing-work-order-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "manufacturing-core:demo",
      exceptionId: "manufacturing-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "manufacturing-core:demo",
      amendedRecordId: "manufacturing-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Manufacturing Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("manufacturing-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "manufacturing-core:demo:amended",
      reversalRecordId: "manufacturing-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("manufacturing-core:demo:amended:reversal");
