...
import { businessFlowDefinitions, publishBom, releaseWorkOrder, recordManufacturingOutput, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("manufacturing-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("manufacturing.boms.publish");
    expect(manifest.publicEvents).toContain("manufacturing.bom-published.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof publishBom).toBe("function");
    expect(typeof releaseWorkOrder).toBe("function");
    expect(typeof recordManufacturingOutput).toBe("function");
